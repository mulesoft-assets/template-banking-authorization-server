...
/**
 *
 * Custom authentication provider that makes external calls to validate
 * username/password combinations. If appropriate Authentication is returned, an
 * OAuth token can be provisioned from the Mule application.
 */
public class ExternalIdResourceOwnerAuthProvider implements AuthenticationProvider {
	...
	/**
	 * Extract username and password from request; call external authentication
	 * provider. Create and return Authentication object if validation is
	 * successful.
	 *
	 * @param credentials
	 *            credentials provided by spring security, from inbound request.
	 *
	...
	public Authentication authenticate(Authentication credentials) throws AuthenticationException {
		...
		if (isUserAuthenticated(userDetails)) {
			...
			UserDetails principal = new ExternalIdServiceUser(userDetails.get(USER_KEY), password, grantedAuths,
					userDetails);
			auth = new UsernamePasswordAuthenticationToken(principal, password, grantedAuths);

			// Adding this to preserve the UserDetails -> AnypointUser that is going to be
			// used by the Custom Token Generator Strategy,
	...
	/**
	 * Utility to check if the JSON response has in fact validated the user as
	 * "AUTHENTICATED"
	 *
	 * @param userDetails
	 *            response returned from the custom HTTP service
	...
	/**
	 * @param externalIdHttpAuthService
	 *            the externalIdHttpAuthService to set
	 */
